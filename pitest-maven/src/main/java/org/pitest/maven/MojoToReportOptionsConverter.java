...
public class MojoToReportOptionsConverter {
    ...
    private ReportOptions parseReportOptions(final List<String> classPath) {
        ...
        try {
            data.setChangedLinesOnFile(parseFileToMapOfChanges(this.mojo.getChanges()));
    ...
    private Map<String, List<Integer>> parseFileToMapOfChanges(File file) throws FileNotFoundException, FileNotFoundException {
        Map<String, List<Integer>> map = new HashMap<>();
        if(file != null) {
            if (file.exists()) {
                Gson gson = new Gson();
                map = gson.fromJson(new FileReader(file), new TypeToken<Map<String, List<Integer>>>() {
                }.getType());
            }
        }
        return map;
